window.view = {
  ...
  proceedToStartButton: function () {
    ...
    if (isNaN(userInput) === false) {
      ...
      } else alert('Enter array size first !');
    } else alert('Size of the array must be an Integer !');
  ...
  createBoxes: function () {
    for (i = 0; i < this.numbers.length; i++) {
      ...
      if (i === 0) element.className = 'sortedArray';
      else if (i === 1) element.className = 'keyPosition';
      else element.className = 'unSortedArray';
  ...
  validateUserInputs: function () {
    var result;
    for (i = 0; i < this.numbers.length; i++) {
      if (isNaN(Number(this.numbers[i]))) return false;
    }
  },
  showImage: function () {
  ...
  displayElements: function () {
    ...
    if (arraySize === this.numbers.length) {
      if (isValidInput === false) alert('Enter Numeric Values Only!');
      else {
        this.createBoxes();
        this.showCode();
    ...
    } else alert('number of inputs must be equal to the size of the array');
  },
  setKey: function () {
  ...
  swapText: function () {
    var elements = document.getElementById('sortingDiv').childNodes;
    elements[this.j + 1].firstChild.innerHTML =
      elements[this.j].firstChild.innerHTML;
    var temp = this.numbers[this.j];
  },
  insertKey: function () {
    var elements = document.getElementById('sortingDiv').childNodes;
    elements[this.j + 1].firstChild.innerHTML = document.getElementById(
      'key'
    ).innerHTML;
  },
  ...
  clearDivs: function () {
    ...
    document.getElementById('inputButtonRadio').className =
      'radioButtonDivision hide';
