...
window.model = {
  ...
  computeSum: function () {
    this.sum =
      this.sum + Math.cos(((2 * Math.PI) / 13) * this.inputValueA) * this.width;
  },
...
window.view = {
  wavelengthController: 0.0472, //control wavelength of cos curve.
  ...
  activateEvents: function () {
    ...
    this.addClickEvent('startBtnId', function () {
      view.startExperiment();
    });
    this.addClickEvent('nextBtnId', function () {
      view.plotCurveArea();
    });
    this.addClickEvent('stopBtnId', function () {
      view.stopExperiment();
    });
  },
  ...
  callDrawRectangle: function () {
    ...
    for (var i = 0; i < 10; i++) {
      dynamicValueOfX++;
      var y =
        70 * Math.cos(0 + (dynamicValueOfX - 20) * this.wavelengthController);
      this.yCoordinatesValue = 70 + (80 - y);
      this.drawRectangle(
        dynamicValueOfX,
        this.yCoordinatesValue,
        1,
        150 - this.yCoordinatesValue
      );
    }
  },
  ...
  validationInput: function () {
    ...
    if (valueA1 === '' || valueB1 === '') {
      ...
      return false;
    } else if (valueA2 < 1 || valueA2 > 10) {
      alert('Invalid input');
      return false;
    } else if (valueB2 < 1 || valueB2 > 10) {
      alert('Invalid input');
      return false;
    } else {
