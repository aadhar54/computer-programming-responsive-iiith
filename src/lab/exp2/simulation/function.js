window.view = {
  ...
  activateClicks: function () {
    this.addClickEvent('imageSquare', function () {
      view.showSquareInputs();
    });
    this.addTouchEvent('imageSquare', function () {
      view.showSquareInputs();
    });
    this.addClickEvent('okSquare', function () {
      view.validateSquareInputs();
    });
    this.addClickEvent('imageRectangle', function () {
      view.showRectangleInputs();
    });
    this.addClickEvent('okRectangle', function () {
      view.validateRectangleInputs();
    });
    this.addClickEvent('imageTriangle', function () {
      view.showTriangleInputs();
    });
    this.addClickEvent('okTriangle', function () {
      view.validateTriangleInputs();
    });
    this.addClickEvent('imageCircle', function () {
      view.showCircleInputs();
    });
    this.addClickEvent('okCircle', function () {
      view.validateCircleInputs();
    });
    this.addClickEvent('btnExecute', function () {
      view.calculateArea();
    });
  },
  ...
  approveRectangleInputs: function () {
    ...
    document.getElementById('questionRectangle').className =
      'questionBlock hide';
  ...
  approveTriangleInputs: function () {
    ...
    this.showInstructions();
    document.getElementById('questionTriangle').className =
      'questionBlock hide';
  ...
  validateRectangleInputs: function () {
    ...
    if (a !== '2')
      alert(
        'Incorrect value of input variables(arguments). Calculating the area of a right angle triangle requires the length of the base and height of the triangle. Try again.'
      );
    else if (b !== 'float')
      alert(
        'Incorrect datatype of input variables(arguments). The value of the sides of a right angled triangle need not be integers. Try again.'
      );
    else if (c !== 'float')
      alert(
        'Incorrect datatype for return type. The value of the area of a right angled triangle need not be an integer. Try again.'
      );
    else if (d !== '(1/2)*a*b')
      alert(
        'Incorrect formula for calculating the area of a right angled triangle. Try again.'
      );
  ...
  displayFunctionForSquare: function () {
    document.getElementById('functionSquare').innerHTML +=
      'float area_sq (float a)<br>{<br> &emsp; float area = a*a;<br>&emsp; return area;<br>}';
  },
  displayFunctionForRectangle: function () {
    document.getElementById('functionRectangle').innerHTML +=
      'float area_rect (float a,float b)<br>{<br> &emsp; float area = (1/2)*a*b;<br> &emsp; return area;<br>}';
  },
  displayFunctionForTriangle: function () {
    document.getElementById('functionTriangle').innerHTML +=
      'float area_triangle (float a)<br>{<br>&emsp;float area = (sqrt(3)/4.0)*a*a;<br>&emsp;return area;<br>}';
  },
  displayFunctionForCircle: function () {
    document.getElementById('functionCircle').innerHTML +=
      'float area_circle (float a)<br>{<br>&emsp;float area = 3.14*a*a;<br>&emsp;return area;<br>}';
  },
    ...
    for (i = 0; i < elements.length; i++) {
      ...
      if (parameter.length === 1) {
        ...
        if (functionCall === 'area_sq()' && isNaN(parameter) === false)
          ...
          functionCall === 'area_triangle()' &&
          isNaN(parameter) === false
        )
        ...
        else alert('Incorrect function call at line ' + String(i + 1));
      } else if (
        parameter.length === 2 &&
        functionCall === 'area_rect()' &&
        isNaN(parameter[0]) === false &&
        isNaN(parameter[1]) === false
      )
    ...
    if (this.correctInputs === 7) {
      alert(
        'Total area is : ' +
          ...
          ' ' +
          ' ' +
          'Correct value is : 132.01738'
      );
      this.resetToInitialState();
    } else alert('Some function calls were incorrect. Please try again !');
  },
  init: function () {
    this.activateClicks();
  },
