window.model = {
  ...
  findexp: function (expression) {
    ...
    for (i = 0; i < expression.length; i++) {
      // returns if expression evaluation is complete
      if (isNaN(expression[i]) && i != 0) {
        isexpfinished = 0;
        break;
      }
    }
    if (isexpfinished == 1) return [-1, -1];
    var sub;
    var symbols = ['%', '*', '/', '-', '+'];
    var closeindex = expression.indexOf(')');

    if (closeindex != -1) {
      // finding the part in bracket to be highlighted
  ...
  firsteval: function (expression, start, end) {
    ...
    else if (sub[i] == '/') c = Math.floor(b / a);
    else if (sub[i] == '%') c = b % a;
    while (
      start - 1 >= 0 &&
      expression[start - 1] == '(' &&
  ...
  findexpbitwise: function (expression) {
    ...
    for (j = 0; j < symbols.length; j++) {
      ...
      for (i = 0; i < sub.length; i++) {
        if (sub[i] == symbols[j]) {
          var tmp = i;
          if (j < 2) i++; // the first two operators exist in pairs
          a = 0;
          var unary = 0;
          //	if( i==0 || (i!=0 && isNaN(sub[i-1])) ) {
          //		continue;
...
window.view = {
  ...
  setEnvironment: function (a, b, c, d, environment, expression) {
    ...
    if (environment === 'logical') {
      document.getElementById('logicalExpressions').className =
        'button loopList';
      ...
      document.getElementById('arithmaticExpressions').className =
        'button loopList';
      document.getElementById('logicalExpressions').className += ' hide';
      document.getElementById('bitwiseExpressions').className += ' hide';
    } else {
      document.getElementById('bitwiseExpressions').className =
        'button loopList';
      document.getElementById('arithmaticExpressions').className += ' hide';
      document.getElementById('logicalExpressions').className += ' hide';
    }
  ...
  setOperatorEnvironment: function () {
    ...
    else this.setEnvironment(2, 5, 10, 11, 'arithmatic', 'a + b - c');
  },
  ...
  validateArithmaticExpression: function (exp) {
    ...
    for (var i = 0; i < exp.length; i++) {
      if (exp[i] === '(') {
        ...
        if (characterOrOperator === 1) {
          ...
          return false;
        } else characterOrOperator = 1;
      } else {
        if (characterOrOperator === 0) {
          alert('You Have Entered  Wrong Expression Syntax !!!');
          return false;
        } else characterOrOperator = 0;
      }
      if (braces < 0) {
        alert(
          'You Have Entered  Wrong Expression Syntax for "()" operator !!!'
        );
        return false;
      }
    }
    if (braces !== 0) {
      alert(
        'You Have Entered Wrong Expression Syntax.\n "()" Operators are not complete !!!'
      );
      return false;
      ...
      if (exp[i] === '(') {
        ...
        if (characterOrOperator === 0) {
          ...
          if (exp[i] == '<' && (i + 1 == exp.length || exp[i + 1] != exp[i])) {
            ...
            exp[i] == '>' &&
            (i + 1 == exp.length || exp[i + 1] != exp[i])
          ) {
            alert('You Have Entered  Wrong Expression Syntax !!!');
            return false;
          } else if (exp[i] == '<' || exp[i] == '>') i++;
        }
      }
      if (braces < 0) {
        alert(
  ...
  printFirstStep: function () {
    ...
    div.innerHTML =
      '<span class = "boldBlue">Exp&nbsp;:&emsp;&emsp;</span>' +
      '[ ' +
      string +
      ' ]';
  ...
  validateExpression: function () {
    ...
    if (isValid) {
      ...
      child.innerHTML =
        'Here, variables are replaced by their values !!<br><br>';
  ...
  printCurrentStep: function () {
    ...
    var stringToEval = this.expression.substring(
      this.starting,
      this.ending + 1
    );
    ...
    exp = exp.replace(
      remainingString,
      '<span class = "peach">' + remainingString
    );
    for (var i = 0; i < exp.length; i++) {
      if (
        exp[i] === '+' ||
    ...
    secondSpan.innerHTML =
      '&emsp;&nbsp;' + '[ ' + exp + ' <span class = "peach"> ]</span>';
  ...
  showIllustration: function (equation) {
    ...
    if (model.isBracket === true) {
      ...
      span.innerHTML =
        ...
        1 +
        ' ) ' +
        'Here brackets have higher precedence than other operators and in case of more than one brackets, the brackets will be solved from left to right fashion.<br>';
      ...
      if (equation.length > 2) {
        ...
        span.innerHTML =
          ...
          'Here ' +
          operator +
          ' have higher precedence than other operators. So, it will be solved first.<br>';
        span.className = 'illustrationText';
        child.appendChild(span);
      }
    }
  ...
  activateEvents: function () {
    this.addClickEvent('buttonSave', function () {
      view.freezeInputs();
    });
    ...
    this.addChangeEvent('operatorList', function () {
      view.setOperatorEnvironment();
    });
    this.addChangeEvent('arithmaticExpressions', function () {
      view.setSelectedEquation('arithmaticExpressions');
    });
    this.addChangeEvent('logicalExpressions', function () {
      view.setSelectedEquation('logicalExpressions');
    });
    this.addChangeEvent('bitwiseExpressions', function () {
      view.setSelectedEquation('bitwiseExpressions');
    });
  },
  init: function () {
    this.activateEvents();
